package utils.convert
{
	...
	public class DateFormat
	{
		...
		static public var DAYS:Array = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
		
		//static public var MONTHS:Array = ["Январь","Февраль","Март","Апрель","Май","Июнь","Июль","Август","Сентябрь","Октябрь","Ноябрь","Декабрь"];
		//static public var DAYS:Array = ["Воскресенье","Понедельник","Вторник","Среда","Четверг","Пятница","Суббота"];
		/**
		 ...
		 */
		static public function formatDate(formatStyle:String, date:Date):String
		{
			...
			{
				dateFormated = dateFormated.replace(/z/g, String(DateFormat.dayOfYear(date.month, date.date)));
			}
		...
		static public function dayOfYear(month:uint, day:uint):uint
		{
			if ((month == 0))
			{
				return day - 1;
		...
		static public function leapYear(year:uint):Boolean
		{
			return ((((year % 4) == 0) && year % 100 != 0) || year % 100 == 0);
		}
		
		static public function numberOfDaysInMonth(month:uint, isLeapYear:Boolean = false):uint
		{
			if ((month == 1))
			{
